 ...
 * 先攻列表命令 .init / .ri
 */
export function registerInitiativeCommands(parent: Command, config: Config, diceAdapter: DiceAdapter) {
  // 先攻列表主命令
  parent.subcommand('init', '先攻列表管理')
  ...
  // 显示先攻列表
  parent.subcommand('init.list', '显示先攻列表')
    .alias('init.show')
    .action(async ({ session }) => {
      try {
        const channelId = session.channelId || session.userId
  ...
  // 清空先攻列表
  parent.subcommand('init.clr', '清空先攻列表')
    .alias('init.clear')
    .action(async ({ session }) => {
      try {
        const channelId = session.channelId || session.userId
  ...
  // 移除先攻条目
  parent.subcommand('init.del <name:text>', '移除先攻条目')
    .alias('init.rm')
  ...
  // 下一个回合
  parent.subcommand('init.next', '下一个回合')
    .alias('init.n')
    .action(async ({ session }) => {
      try {
        const channelId = session.channelId || session.userId
  ...
  // 快速先攻 .ri
  parent.subcommand('ri [modifier:number]', '快速先攻检定')
