 ...
 * WOD骰池命令 .ww
 */
export function registerWODCommands(parent: Command, config: Config, diceAdapter: DiceAdapter) {
  // .ww 骰池（显示详细）
  parent.subcommand('ww <expression:text>', 'WOD骰池（显示详细）')
  ...
  // .w 骰池（只显示结果）
  parent.subcommand('w <expression:text>', 'WOD骰池（只显示结果）')
    .action(async ({ session }, expression) => {
      if (!expression) {
 ...
 * 解析并投掷WOD骰池
 */
function parseAndRollWOD(expression: string, diceAdapter: DiceAdapter, showDetail: boolean): string {
  // 解析表达式 例如: 10a8
  ...
  const againLine = parseInt(match[2])
